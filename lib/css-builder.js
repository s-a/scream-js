			...
			for (var i = 0; i < images.length; i++) {
				...
				for (var r = 0; r < image.image.meta.length; r++) {
					...
					for (var i2 = i - 1; i2 >= 0; i2--) {
						...
						var image2 = images[i]; 
						var meta2 = getMetaDataByPixelRatio(meta.pixelRatio, image2.image.meta);
						top += meta2.height;
						space += 0;
					}
					meta.top = (top + space) * -1;
				}
			}			
		}
	};
	...
	var buildCssItems = function () {
		...
		for ( var alias in config.screamSettings.images ) {
			...
			if (img.sprite){  
				...
				result.push('\twidth: ' + originalMeta.width + 'px;');
				result.push('\theight: ' + originalMeta.height + 'px;'); 
				result.push('\tbackground-size: cover;');					
				result.push('\tbackground-position:0 ' + (originalMeta.top) + 'px;');  
				result.push("}\n");
			}  
		}
		return result;
